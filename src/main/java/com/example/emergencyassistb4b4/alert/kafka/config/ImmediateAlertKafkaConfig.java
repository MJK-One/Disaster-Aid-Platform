...
public class ImmediateAlertKafkaConfig { // 즉시 알림 처리용 Kafka 리스너 설정
    ...
    public ConcurrentKafkaListenerContainerFactory<String, DisasterReportedEvent> immediateListenerFactory() {
        ...
        factory.getContainerProperties().setAckMode(ContainerProperties.AckMode.RECORD); // 메시지 단위 수동 커밋
        factory.setCommonErrorHandler(kafkaBaseConfig.defaultErrorHandler(kafkaTemplate)); // 공통 에러 핸들러 등록
